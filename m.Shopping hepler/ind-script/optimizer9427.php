...
var globalPictorialSetUi = function(module, seq) {
    ...
    var $data = $.parseJSON(decodeURIComponent(module.find('li').eq(seq).data('infomation'))),
        ...
        $sale_price = $infomation.find('p.sale_price').children('a'),
        $sale_value = $sale_price.find('span.sale_value'),
    ...
    if (module.find('li').size() < 1) {
        ...
        $name.add($price).add($detail).unbind().bind('click', function(){ pictorialDetailview($data.param); });
        // set product infomation
        $name.text(parseHtmlEnteties($data.product_name));
        $price_value.text(parseHtmlEnteties($data.product_price));
        $price_ref.text(parseHtmlEnteties($data.txt_product_price_ref));

        $price_value.removeClass('strike');
        $price_ref.removeClass('strike');
        $sale_value.addClass('displaynone');

        if ($sale_price.length > 0 && $data.sale_price.length > 0) {
            // 할인가 출력
            if ($data.sale_price_display === true) {
                $sale_value.text(parseHtmlEnteties($data.sale_price));
                $sale_value.removeClass('displaynone');

                // 판매가 취소선 표시
                if ($data.product_price_strike === 'T') {
                    $price_value.addClass('strike');
                    $price_ref.addClass('strike');
                }

                if ($data.product_tax_type_text_display === true) {
                    $sale_value.append($('<strong></strong>').html($data.product_tax_type_text));
                }
            }
        } else {
            if ($data.product_tax_type_text_display === true) {
                $price_value.append($('<strong></strong>').html($data.product_tax_type_text));
            }
        }

        if (product_price_ref_display != '') { $price_ref.addClass(product_price_ref_display); }

        // 모바일 > 화보형 목록 > 상품 좋아요 버튼(아이콘) 세팅
        LIKE_BUTTON_PRD_ZOOM.setLikeButton($data, $infomation);
    }
}

/**
 * 모바일 > 화보형 목록 > 상품 좋아요 버튼(아이콘)
 */
var LIKE_BUTTON_PRD_ZOOM = {
    aLoadedLikeButton : [],  // 좋아요 버튼(아이콘) 엘리먼트 캐싱

    // 상품 좋아요 버튼(아이콘) 세팅
    setLikeButton     : function($data, $infomation) {
        // 좋아요 사용여부
        var iPrdNo = parseInt($($data.disp_likeprd_icon).attr('product_no'), 10);
        if ($data.disp_likeprd_class === 'displaynone' || iPrdNo < 1) {
            return;
        }

        // 캐싱된 좋아요 버튼 유무 확인 및 적용
        if (this.aLoadedLikeButton[iPrdNo]) {
            $infomation.find('div.likeButton').remove();
            $infomation.append(this.aLoadedLikeButton[iPrdNo]);
            return;
        }

        EC_SHOP_FRONT_NEW_LIKE_COMMON.getMyLikeProductNoInList(function(aData) {
            var sWrapperClassName, sImgSrc, sIconStatus;

            var bIsMyLikeProduct = false;
            for (var i = 0, iLength = aData.rows.length; i < iLength; i++) {
                if (aData.rows[i].product_no === iPrdNo) {
                    bIsMyLikeProduct = true;
                    break;
                }
            }

            if (bIsMyLikeProduct === true) {
                sWrapperClassName = 'likeButton selected likePrd likePrd_'+iPrdNo;
                sImgSrc = aData.imgSrc.on;
                sIconStatus = 'on';
            } else {
                sWrapperClassName = 'likeButton likePrd likePrd_'+iPrdNo;
                sImgSrc = aData.imgSrc.off;
                sIconStatus = 'off';
            }

            var oNodeLikeWrapper = $('<div></div>').addClass(sWrapperClassName);
            var oNodeLikeButton  = $('<button type="button"></button>');
            var oNodeImg = $('<img />').attr({
                'class' : 'likePrdIcon',
                'src' : sImgSrc,
                'product_no' : iPrdNo,
                'icon_status' : sIconStatus
            });
            var oNodeStrong = $('<strong></strong>').html($data.disp_likeprd_count);
            oNodeLikeButton.append(oNodeImg);
            oNodeLikeButton.append(oNodeStrong);
            oNodeLikeWrapper.append(oNodeLikeButton);

            // 화면 출력
            $infomation.find('div.likeButton').remove(); // 이전화보의 좋아요버튼 삭제
            $infomation.append(oNodeLikeWrapper);        // 현재화보의 좋아요버튼 삽입

            // 캐싱
            LIKE_BUTTON_PRD_ZOOM.aLoadedLikeButton[iPrdNo] = oNodeLikeWrapper;
        });
    }
};
        ...
        var $effect = {
            ...
            setObject : function(){
                try {
                    ...
                    // prev button
                    this.$prev = $(sActiveProduct).find('div.pagenate').find('button#pictorial_prev_' + this.iActive);
                    // next button
                    this.$next = $(sActiveProduct).find('div.pagenate').find('button#pictorial_next_' + this.iActive);
                } catch(e) { }
            },
            /**
             * set total page
            ...
            setEvent : function() {
                this.$event = {
                     ...
                     */
                    action : function(){
                        $effect.$next.unbind().bind('click', function(e){ e.stopPropagation(); $effect.next(); });
                        $effect.$prev.unbind().bind('click', function(e){ e.stopPropagation(); $effect.prev(); });
